...
val MANIFEST = listOf(
    ...
    "$FACET:create",  // add a task
    "$FACET:update",  // edit one, tick one off, roll a repeating one forward
 ...
 * the right, so `tasks:read` does not cover `tasks:*`.
 */
...
data class Grants(val held: List<String>?) {
    ...
    /** Read granted and nothing else. The app shows every task and
