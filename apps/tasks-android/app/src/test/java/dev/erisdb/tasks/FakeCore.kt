 ...
 */
class FakeCore(private val facet: String = "tasks") : CoreApi {
    ...
    var grants: List<String> = listOf("tasks:read", "tasks:create", "tasks:update", "tasks:delete")

    /** Facet actions this token is refused, as the core refuses them. */
    var forbidden = setOf<String>()

    /** True while the tasks facet is registered. When it is not, writes
    ...
    /** A `lapsed` row, as the core's sweep emits one. */
    fun lapse(id: String) {
        record(id, "lapsed", items[id])
    }
