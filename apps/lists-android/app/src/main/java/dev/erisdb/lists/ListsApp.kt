 ...
 */
@Composable
fun ListsApp(pairUri: String? = null, onPairHandled: () -> Unit = {}) {
    ...
    var grid by remember { mutableStateOf(store.grid) }
    var linkPreviews by remember { mutableStateOf(store.linkPreviews) }
    ...
    val shown = remember(snapshot, outbox, aliases) { applyPending(snapshot?.items?.values.orEmpty(), outbox, aliases) }
    ...
    // to a lists screen this phone may not have earned yet.
    ...
    CompositionLocalProvider(LocalLinkPreviews provides linkPreviews) {
    AnimatedContent(
        targetState = screen,
        transitionSpec = {
                ...
                if (s.item == null) {
                    ...
                    mutate(
                        JSONObject()
                            ...
                            .put("revision", s.item.optLong("revision"))
                            .put("body", body)
                    )
                }
                screen = Screen.Main
        ...
        is Screen.Settings -> SettingsScreen(
            ...
            onLinkPreviews = { linkPreviews = it; store.linkPreviews = it },
                ...
                Text(
                    if (pending.sameCore) {
                        "This code is for $here, the erisdb already paired. Redeeming it " +
                            ...
                            "capability with whatever is approved; the entries stay where " +
                            "they are."
                    } else {
                        "This phone is paired with $here. Pairing with $there drops the entries " +
